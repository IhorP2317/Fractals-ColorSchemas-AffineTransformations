  ...
  for (let x = 0; x < canvas.width; x++) {
    for (let y = 0; y < canvas.height; y++) {
      ...
      while (iteration < iterations) {
        ...
        // Fractal sin(z) * cos(z)
        const newA =
          Math.sin(a) * Math.cos(a) - Math.sinh(b) * Math.cosh(b) + zx;
        const newB =
          Math.sin(a) * Math.cosh(b) + Math.cos(a) * Math.sinh(b) + zy;
...
function drawCezaroFractal(zoom, iteration, hue) {
  ...
  const initialLength =
    Math.min(canvas.width, canvas.height) * zoom * Math.pow(0.725, -iteration);
